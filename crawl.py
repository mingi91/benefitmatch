...
"김포시": "경기", "김포": "경기",
"광명시": "경기", "광명": "경기",
"광주시": "경기", "광주": "경기",   # ⚠ 광주광역시와 혼동 가능 → 그대로 둘지 고려 필요
"군포시": "경기", "군포": "경기",
"이천시": "경기", "이천": "경기",
"오산시": "경기", "오산": "경기",
"양주시": "경기", "양주": "경기",
...
"사하구": "부산", "사하": "부산",
"연제구": "부산", "연제": "부산",
"남구": "전국", "동구": "전국", "서구": "전국", "북구": "전국",  # 모호 → 유지
"강서구": "부산 강서",  # → 특별 처리 필요 (서울 강서와 구분)
"금정구": "부산", "금정": "부산",
"기장군": "부산", "기장": "부산",
...
def extract_region(agency_name: str):
    ...
    # ---------------- 특수 케이스 패턴 처리 ----------------
    # 강서구 (서울/부산 구분)
    if re.search(r"서울.*강서구", agency_name):
        return "서울", "강서구"
    if re.search(r"부산.*강서구", agency_name):
        return "부산", "강서구"

    # 광주시 (경기 vs 광주광역시)
    if re.search(r"경기.*광주시", agency_name):
        return "경기", "광주시"
    if re.search(r"광주광역시.*광주시", agency_name):
        return "광주", "광주시"
    # -------------------------------------------------------
